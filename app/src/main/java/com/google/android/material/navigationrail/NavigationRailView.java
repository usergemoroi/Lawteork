...
public class NavigationRailView extends NavigationBarView {
    ...
    public NavigationRailView(Context context, AttributeSet attrs) {
        this(context, attrs, C0605R.attr.navigationRailStyle);
    }

    public NavigationRailView(Context context, AttributeSet attrs, int defStyleAttr) {
        this(context, attrs, defStyleAttr, C0605R.style.Widget_MaterialComponents_NavigationRailView);
    }
    ...
    public NavigationRailView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        this.topMargin = res.getDimensionPixelSize(C0605R.dimen.mtrl_navigation_rail_margin);
        Context context2 = getContext();
        TintTypedArray attributes = ThemeEnforcement.obtainTintedStyledAttributes(context2, attrs, C0605R.styleable.NavigationRailView, defStyleAttr, defStyleRes, new int[0]);
        int headerLayoutRes = attributes.getResourceId(C0605R.styleable.NavigationRailView_headerLayout, 0);
        if (headerLayoutRes != 0) {
            addHeaderView(headerLayoutRes);
        }
        setMenuGravity(attributes.getInt(C0605R.styleable.NavigationRailView_menuGravity, 49));
        if (attributes.hasValue(C0605R.styleable.NavigationRailView_itemMinHeight)) {
            setItemMinimumHeight(attributes.getDimensionPixelSize(C0605R.styleable.NavigationRailView_itemMinHeight, -1));
        }
        if (attributes.hasValue(C0605R.styleable.NavigationRailView_paddingTopSystemWindowInsets)) {
            this.paddingTopSystemWindowInsets = Boolean.valueOf(attributes.getBoolean(C0605R.styleable.NavigationRailView_paddingTopSystemWindowInsets, false));
        }
        if (attributes.hasValue(C0605R.styleable.NavigationRailView_paddingBottomSystemWindowInsets)) {
            this.paddingBottomSystemWindowInsets = Boolean.valueOf(attributes.getBoolean(C0605R.styleable.NavigationRailView_paddingBottomSystemWindowInsets, false));
        }
        attributes.recycle();
        applyWindowInsets();
    }
        ...
        ViewUtils.doOnApplyWindowInsets(this, new ViewUtils.OnApplyWindowInsetsListener() { // from class: com.google.android.material.navigationrail.NavigationRailView.1
            ...
            public WindowInsetsCompat onApplyWindowInsets(View view, WindowInsetsCompat insets, ViewUtils.RelativePadding initialPadding) {
                ...
                if (navigationRailView.shouldApplyWindowInsetPadding(navigationRailView.paddingTopSystemWindowInsets)) {
                    initialPadding.f258top += insets.getInsets(WindowInsetsCompat.Type.systemBars()).f255top;
                }
