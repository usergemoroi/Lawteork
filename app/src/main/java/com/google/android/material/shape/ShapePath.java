...
public class ShapePath {
    ...
    public void addArc(float left, float top2, float right, float bottom, float startAngle, float sweepAngle) {
        PathArcOperation operation = new PathArcOperation(left, top2, right, bottom);
        ...
        setEndY(((top2 + bottom) * 0.5f) + (((bottom - top2) / 2.0f) * ((float) Math.sin(Math.toRadians(startAngle + sweepAngle)))));
    }
    ...
    public static class LineShadowOperation extends ShadowCompatOperation {
        ...
        public void draw(Matrix transform, ShadowRenderer shadowRenderer, int shadowElevation, Canvas canvas) {
            float height = this.operation.f148y - this.startY;
            float width = this.operation.f147x - this.startX;
        ...
        float getAngle() {
            return (float) Math.toDegrees(Math.atan((this.operation.f148y - this.startY) / (this.operation.f147x - this.startX)));
        }
    }
    ...
    static class InnerCornerShadowOperation extends ShadowCompatOperation {
        ...
        public void draw(Matrix transform, ShadowRenderer shadowRenderer, int shadowElevation, Canvas canvas) {
            ...
            if (sweepAngle <= 0.0f) {
                double length1 = Math.hypot(this.operation1.f147x - this.startX, this.operation1.f148y - this.startY);
                double length22 = Math.hypot(this.operation2.f147x - this.operation1.f147x, this.operation2.f148y - this.operation1.f148y);
                ...
                this.renderMatrix.set(transform);
                this.renderMatrix.preTranslate(this.operation1.f147x, this.operation1.f148y);
                this.renderMatrix.preRotate(getStartAngle());
        ...
        float getStartAngle() {
            return (float) Math.toDegrees(Math.atan((this.operation1.f148y - this.startY) / (this.operation1.f147x - this.startX)));
        }

        float getEndAngle() {
            return (float) Math.toDegrees(Math.atan((this.operation2.f148y - this.operation1.f148y) / (this.operation2.f147x - this.operation1.f147x)));
        }
    ...
    public static class PathLineOperation extends PathOperation {
        ...
        public void applyToPath(Matrix transform, Path path) {
            ...
            path.lineTo(this.f147x, this.f148y);
    ...
    public static class PathArcOperation extends PathOperation {
        ...
        public float f259top;

        public PathArcOperation(float left, float top2, float right, float bottom) {
            setLeft(left);
            setTop(top2);
            setRight(right);
            setBottom(bottom);
