...
public class MaterialDividerItemDecoration extends RecyclerView.ItemDecoration {
    private static final int DEF_STYLE_RES = C0605R.style.Widget_MaterialComponents_MaterialDivider;
    ...
    public MaterialDividerItemDecoration(Context context, AttributeSet attrs, int orientation) {
        this(context, attrs, C0605R.attr.materialDividerStyle, orientation);
    }

    public MaterialDividerItemDecoration(Context context, AttributeSet attrs, int defStyleAttr, int orientation) {
        this.tempRect = new Rect();
        TypedArray attributes = ThemeEnforcement.obtainStyledAttributes(context, attrs, C0605R.styleable.MaterialDivider, defStyleAttr, DEF_STYLE_RES, new int[0]);
        this.color = MaterialResources.getColorStateList(context, attributes, C0605R.styleable.MaterialDivider_dividerColor).getDefaultColor();
        this.thickness = attributes.getDimensionPixelSize(C0605R.styleable.MaterialDivider_dividerThickness, context.getResources().getDimensionPixelSize(C0605R.dimen.material_divider_thickness));
        this.insetStart = attributes.getDimensionPixelOffset(C0605R.styleable.MaterialDivider_dividerInsetStart, 0);
        this.insetEnd = attributes.getDimensionPixelOffset(C0605R.styleable.MaterialDivider_dividerInsetEnd, 0);
        this.lastItemDecorated = attributes.getBoolean(C0605R.styleable.MaterialDivider_lastItemDecorated, true);
        attributes.recycle();
    ...
    private void drawForVerticalOrientation(Canvas canvas, RecyclerView parent) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (shouldDrawDivider(parent, child)) {
                ...
                int top2 = bottom - this.thickness;
                this.dividerDrawable.setBounds(left2, top2, right2, bottom);
    ...
    private void drawForHorizontalOrientation(Canvas canvas, RecyclerView parent) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (shouldDrawDivider(parent, child)) {
                ...
                this.dividerDrawable.setBounds(left, top3, right, bottom2);
                this.dividerDrawable.draw(canvas);
            }
        }
        canvas.restore();
