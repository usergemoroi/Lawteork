...
public class WaitingRequestManager implements Request.NetworkRequestCompleteListener {
    ...
    public void onResponseReceived(Request<?> request, Response<?> response) {
        ...
        if (waitingRequests != null) {
            if (VolleyLog.DEBUG) {
                VolleyLog.m145v("Releasing %d waiting requests for cacheKey=%s.", Integer.valueOf(waitingRequests.size()), cacheKey);
            }
    ...
    public synchronized void onNoUsableResponseReceived(Request<?> request) {
        ...
        if (waitingRequests != null && !waitingRequests.isEmpty()) {
            if (VolleyLog.DEBUG) {
                VolleyLog.m145v("%d waiting requests for cacheKey=%s; resend to network", Integer.valueOf(waitingRequests.size()), cacheKey);
            }
            ...
            if (requestQueue != null) {
                ...
                try {
                    ...
                    VolleyLog.m147e("Couldn't add request to queue. %s", iex.toString());
    ...
    public synchronized boolean maybeAddToWaitingRequests(Request<?> request) {
        ...
        if (this.mWaitingRequests.containsKey(cacheKey)) {
            ...
            if (VolleyLog.DEBUG) {
                VolleyLog.m148d("Request for cacheKey=%s is in flight, putting on hold.", cacheKey);
            }
            return true;
        }
        ...
        if (VolleyLog.DEBUG) {
            VolleyLog.m148d("new request, sending to network %s", cacheKey);
        }
        return false;
    }
}
