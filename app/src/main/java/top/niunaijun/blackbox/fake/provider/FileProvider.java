...
public class FileProvider extends ContentProvider {
    ...
    public static class SimplePathStrategy implements PathStrategy {
        ...
        @Override // top.niunaijun.blackbox.fake.provider.FileProvider.PathStrategy
        ...
        public Uri getUriForFile(File file) {
            try {
                ...
                if (entry != null) {
                    ...
                    boolean endsWith = path2.endsWith(InternalZipConstants.ZIP_FILE_SEPARATOR);
                    int length = path2.length();
                    if (!endsWith) {
                        length++;
                    }
                    return new Uri.Builder().scheme("content").authority(this.mAuthority).encodedPath(Uri.encode(entry.getKey()) + '/' + Uri.encode(canonicalPath.substring(length), InternalZipConstants.ZIP_FILE_SEPARATOR)).build();
                }
