    ...
    */
    private final void updateHead(Subscriber<E> subscriber, Subscriber<E> subscriber2) {
        ...
        while (true) {
            ...
            if (subscriber3 != null) {
                try {
                    subscriber3.setSubHead(getTail());
                    boolean wasEmpty = this.subscribers.isEmpty();
                    this.subscribers.add(subscriber3);
                    if (!wasEmpty) {
                        reentrantLock.unlock();
                        return;
                    }
            ...
            if (subscriber4 != null) {
                this.subscribers.remove(subscriber4);
                if (getHead() != subscriber4.getSubHead()) {
                    reentrantLock.unlock();
                    return;
                }
            }
