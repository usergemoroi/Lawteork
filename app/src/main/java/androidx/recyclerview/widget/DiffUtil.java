...
public class DiffUtil {
    private static final Comparator<Snake> SNAKE_COMPARATOR = new Comparator<Snake>() { // from class: androidx.recyclerview.widget.DiffUtil.1
        ...
        public int compare(Snake o1, Snake o2) {
            int cmpX = o1.f83x - o2.f83x;
            return cmpX == 0 ? o1.f84y - o2.f84y : cmpX;
        }
    };
    ...
    public static DiffResult calculateDiff(Callback cb, boolean detectMoves) {
        ...
        while (!stack.isEmpty()) {
            ...
            if (snake != null) {
                ...
                snake.f83x += range.oldListStart;
                snake.f84y += range.newListStart;
                Range left = rangePool.isEmpty() ? new Range() : rangePool.remove(rangePool.size() - 1);
                left.oldListStart = range.oldListStart;
                left.newListStart = range.newListStart;
                if (snake.reverse) {
                    left.oldListEnd = snake.f83x;
                    left.newListEnd = snake.f84y;
                } else if (snake.removal) {
                    left.oldListEnd = snake.f83x - 1;
                    left.newListEnd = snake.f84y;
                } else {
                    left.oldListEnd = snake.f83x;
                    left.newListEnd = snake.f84y - 1;
                }
                stack.add(left);
                if (!snake.reverse) {
                    range.oldListStart = snake.f83x + snake.size;
                    range.newListStart = snake.f84y + snake.size;
                } else if (snake.removal) {
                    range.oldListStart = snake.f83x + snake.size + 1;
                    range.newListStart = snake.f84y + snake.size;
                } else {
                    range.oldListStart = snake.f83x + snake.size;
                    range.newListStart = snake.f84y + snake.size + 1;
                }
                stack.add(range);
            } else {
                rangePool.add(range);
            ...
            for (int d = 0; d <= dLimit; d++) {
                for (int k = -d; k <= d; k += 2) {
                    ...
                    if (checkInFwd && k >= (delta - d) + 1 && k <= (delta + d) - 1 && forward[kOffset + k] >= backward[kOffset + k]) {
                        ...
                        outSnake.f83x = backward[kOffset + k];
                        outSnake.f84y = outSnake.f83x - k;
                ...
                while (k2 <= d) {
                    ...
                    if (checkInFwd || k2 + delta < (-d) || k2 + delta > d || forward[kOffset + backwardK] < backward[kOffset + backwardK]) {
                        ...
                        outSnake2.f83x = backward[kOffset + backwardK];
                        outSnake2.f84y = outSnake2.f83x - backwardK;
    ...
    public static class DiffResult {
        ...
        private void addRootSnake() {
            ...
            if (firstSnake == null || firstSnake.f83x != 0 || firstSnake.f84y != 0) {
        ...
        private void findMatchingItems() {
            ...
            for (int i = this.mSnakes.size() - 1; i >= 0; i--) {
                ...
                int endX = snake.f83x + snake.size;
                int endY = snake.f84y + snake.size;
                ...
                for (int j = 0; j < snake.size; j++) {
                    int oldItemPos = snake.f83x + j;
                    int newItemPos = snake.f84y + j;
        ...
        public void dispatchUpdatesTo(ListUpdateCallback updateCallback) {
            ...
            for (int snakeIndex = this.mSnakes.size() - 1; snakeIndex >= 0; snakeIndex--) {
                ...
                int endX = snake.f83x + snakeSize2;
                int endY2 = snake.f84y + snakeSize2;
                if (endX >= posOld2) {
                    endY = endY2;
                } else {
                    endY = endY2;
                ...
                for (int i = snakeSize - 1; i >= 0; i--) {
                    if ((this.mOldItemStatuses[snake.f83x + i] & 31) == 2) {
                        batchingCallback.onChanged(snake.f83x + i, 1, this.mCallback.getChangePayload(snake.f83x + i, snake.f84y + i));
                    }
                }
                posOld2 = snake.f83x;
                posNew2 = snake.f84y;
            ...
            for (int i = count - 1; i >= 0; i--) {
                ...
                switch (status) {
                    ...
                    case 16:
                        ...
                        break;
                    default:
                        throw new IllegalStateException("unknown flag for pos " + (globalIndex + i) + ILogger.MSG_SEPARATOR + Long.toBinaryString(status));
                }
            }
        }
