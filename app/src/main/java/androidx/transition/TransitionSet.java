...
public class TransitionSet extends Transition {
    ...
    public String toString(String indent) {
        ...
        for (int i = 0; i < this.mTransitions.size(); i++) {
            result = result + ShellUtils.COMMAND_LINE_END + this.mTransitions.get(i).toString(indent + "  ");
        }
        return result;
    }
    ...
    @Override // androidx.transition.Transition
    /* renamed from: clone */
    public Transition mo235clone() {
        TransitionSet clone = (TransitionSet) super.mo235clone();
        clone.mTransitions = new ArrayList<>();
        int numTransitions = this.mTransitions.size();
        for (int i = 0; i < numTransitions; i++) {
            clone.addTransitionInternal(this.mTransitions.get(i).mo235clone());
        }
        return clone;
    }
}
