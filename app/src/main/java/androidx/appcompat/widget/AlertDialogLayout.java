    ...
    private boolean tryOnMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (child.getVisibility() != 8) {
                int id = child.getId();
                if (id == C0030R.C0033id.topPanel) {
                    topPanel = child;
                } else if (id == C0030R.C0033id.buttonPanel) {
                    buttonPanel = child;
                } else if ((id != C0030R.C0033id.contentPanel && id != C0030R.C0033id.customPanel) || middlePanel != null) {
                    return false;
                } else {
                    middlePanel = child;
                }
    ...
    public void onLayout(boolean changed, int left, int top2, int right, int bottom) {
        ...
        switch (majorGravity) {
            case 16:
                ...
                childTop = childTop2 + (((bottom - top2) - totalLength) / 2);
                break;
            case 80:
                int childTop3 = getPaddingTop();
                childTop = ((childTop3 + bottom) - top2) - totalLength;
                break;
            default:
