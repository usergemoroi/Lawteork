...
public class SolverVariableValues implements ArrayRow.ArrayRowVariables {
    ...
    public void display() {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (v != null) {
                System.out.print(v + " = " + getVariableValue(i) + ILogger.MSG_SEPARATOR);
            }
        }
        System.out.println(" }");
    }
    ...
    public String toString() {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (v != null) {
                int index = indexOf(v);
                String str4 = (str3 + v + " = " + getVariableValue(i) + ILogger.MSG_SEPARATOR) + "[p: ";
    ...
    private void addToHashMap(SolverVariable variable, int index) {
        int[] iArr;
        int hash = variable.f35id % this.HASH_SIZE;
        int[] iArr2 = this.keys;
        int key = iArr2[hash];
        if (key == -1) {
            iArr2[hash] = index;
    ...
    private void displayHash() {
        for (int i = 0; i < this.HASH_SIZE; i++) {
            if (this.keys[i] != -1) {
                ...
                boolean done = false;
                while (!done) {
                    str = str + ILogger.MSG_SEPARATOR + this.variables[key];
                    int[] iArr = this.nextKeys;
                    if (iArr[key] != -1) {
                        key = iArr[key];
                    } else {
    ...
    private void addVariable(int index, SolverVariable variable, float value) {
        this.variables[index] = variable.f35id;
    ...
    public void put(SolverVariable variable, float value) {
        ...
        if (value > (-f) && value < f) {
            ...
            int j = this.head;
            for (int i = 0; i < count; i++) {
                if (this.variables[j] == variable.f35id) {
                    this.values[j] = value;
                    return;
                }
                if (this.variables[j] < variable.f35id) {
