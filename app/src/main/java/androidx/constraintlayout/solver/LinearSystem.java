...
public class LinearSystem {
    ...
    public SolverVariable createObjectVariable(Object anchor) {
        ...
        if (anchor instanceof ConstraintAnchor) {
            ...
            if (variable.f35id == -1 || variable.f35id > this.mVariablesID || this.mCache.mIndexedVariables[variable.f35id] == null) {
                if (variable.f35id != -1) {
                    variable.reset();
                }
    ...
    private final int optimize(Row goal, boolean b) {
        ...
        while (!done) {
            ...
            if (goal.getKey() != null) {
                this.mAlreadyTestedCandidates[goal.getKey().f35id] = true;
            }
            SolverVariable pivotCandidate = goal.getPivotCandidate(this, this.mAlreadyTestedCandidates);
            if (pivotCandidate != null) {
                if (this.mAlreadyTestedCandidates[pivotCandidate.f35id]) {
                    return tries;
                }
                this.mAlreadyTestedCandidates[pivotCandidate.f35id] = true;
            }
            if (pivotCandidate != null) {
    ...
    private void displayRows() {
        ...
        String s = "";
        for (int i = 0; i < this.mNumRows; i++) {
            s = (s + this.mRows[i]) + ShellUtils.COMMAND_LINE_END;
        }
        System.out.println(s + this.mGoal + ShellUtils.COMMAND_LINE_END);
    }
    ...
    public void displayReadableRows() {
        ...
        for (int i = 0; i < this.mVariablesID; i++) {
            ...
            if (variable != null && variable.isFinalValue) {
                s = s + " $[" + i + "] => " + variable + " = " + variable.computedValue + ShellUtils.COMMAND_LINE_END;
            }
        }
        String s2 = s + "\n\n #  ";
        for (int i2 = 0; i2 < this.mNumRows; i2++) {
        ...
        if (this.mGoal != null) {
            s2 = s2 + "Goal: " + this.mGoal + ShellUtils.COMMAND_LINE_END;
        }
    ...
    public void displayVariablesReadableRows() {
        ...
        for (int i = 0; i < this.mNumRows; i++) {
            if (this.mRows[i].variable.mType == SolverVariable.Type.UNRESTRICTED) {
                s = (s + this.mRows[i].toReadableString()) + ShellUtils.COMMAND_LINE_END;
            }
        }
        System.out.println(s + this.mGoal + ShellUtils.COMMAND_LINE_END);
    }
    ...
    void displaySystemInformations() {
        ...
        printStream.println(append.append(getDisplaySize(i3 * i3)).append(") -- row sizes: ").append(getDisplaySize(rowSize)).append(", actual size: ").append(getDisplaySize(actualRowSize)).append(" rows: ").append(this.mNumRows).append(InternalZipConstants.ZIP_FILE_SEPARATOR).append(this.mMaxRows).append(" cols: ").append(this.mNumColumns).append(InternalZipConstants.ZIP_FILE_SEPARATOR).append(this.mMaxColumns).append(ILogger.MSG_SEPARATOR).append(0).append(" occupied cells, ").append(getDisplaySize(0)).toString());
    }
