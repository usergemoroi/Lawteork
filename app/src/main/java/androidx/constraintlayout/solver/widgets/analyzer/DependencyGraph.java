...
public class DependencyGraph {
    ...
    private String generateDisplayNode(DependencyNode node, boolean centeredConnection, String content) {
        for (DependencyNode target : node.targets) {
            String constraint = (ShellUtils.COMMAND_LINE_END + node.name()) + " -> " + target.name();
            ...
            content = content + (constraint + ShellUtils.COMMAND_LINE_END);
        }
        return content;
    }
    ...
    private String nodeDefinition(WidgetRun run) {
        ...
        String group = "";
        if (runGroup != null) {
            group = " [" + (runGroup.groupIndex + 1) + InternalZipConstants.ZIP_FILE_SEPARATOR + RunGroup.index + "]";
        }
        ...
        if ((root instanceof HorizontalWidgetRun) || ((root instanceof ChainRun) && ((ChainRun) root).orientation == 0)) {
            ...
            if (behaviour == ConstraintWidget.DimensionBehaviour.FIXED || behaviour == ConstraintWidget.DimensionBehaviour.WRAP_CONTENT) {
                if (!start.targets.isEmpty() && end.targets.isEmpty()) {
                    String constraint = ShellUtils.COMMAND_LINE_END + end.name() + " -> " + start.name() + ShellUtils.COMMAND_LINE_END;
                    content2 = content2 + constraint;
                } else if (start.targets.isEmpty() && !end.targets.isEmpty()) {
                    String constraint2 = ShellUtils.COMMAND_LINE_END + start.name() + " -> " + end.name() + ShellUtils.COMMAND_LINE_END;
                ...
                String str = ShellUtils.COMMAND_LINE_END + name + "_HORIZONTAL -> " + name + "_VERTICAL;\n";
            }
            ...
            if (behaviour2 == ConstraintWidget.DimensionBehaviour.FIXED || behaviour2 == ConstraintWidget.DimensionBehaviour.WRAP_CONTENT) {
                if (!start.targets.isEmpty() && end.targets.isEmpty()) {
                    String constraint3 = ShellUtils.COMMAND_LINE_END + end.name() + " -> " + start.name() + ShellUtils.COMMAND_LINE_END;
                    content2 = content2 + constraint3;
                } else if (start.targets.isEmpty() && !end.targets.isEmpty()) {
                    String constraint4 = ShellUtils.COMMAND_LINE_END + start.name() + " -> " + end.name() + ShellUtils.COMMAND_LINE_END;
                ...
                String str2 = ShellUtils.COMMAND_LINE_END + name2 + "_VERTICAL -> " + name2 + "_HORIZONTAL;\n";
            }
        }
