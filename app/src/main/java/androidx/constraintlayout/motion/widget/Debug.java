...
public class Debug {
    public static void logStack(String tag, String msg, int n) {
        ...
        String s = ILogger.MSG_SEPARATOR;
        ...
        for (Field declaredField : declaredFields) {
            try {
                ...
                if (declaredField.getName().startsWith("layout_constraint") && ((!(value instanceof Integer) || !value.toString().equals("-1")) && ((!(value instanceof Integer) || !value.toString().equals("0")) && ((!(value instanceof Float) || !value.toString().equals(BuildConfig.VERSION_NAME)) && (!(value instanceof Float) || !value.toString().equals("0.5")))))) {
                    System.out.println(loc + "    " + declaredField.getName() + ILogger.MSG_SEPARATOR + value);
                }
    ...
    public static String getName(Context context, int[] id) {
        ...
        try {
            ...
            int i = 0;
            while (i < id.length) {
                String str3 = str2 + (i == 0 ? "" : ILogger.MSG_SEPARATOR);
                try {
                    str = context.getResources().getResourceEntryName(id[i]);
                } catch (Resources.NotFoundException e) {
                    str = "? " + id[i] + ILogger.MSG_SEPARATOR;
                }
                String tmp = str;
                str2 = str3 + tmp;
                i++;
    ...
    public static void dumpLayoutParams(ViewGroup.LayoutParams param, String str) {
        ...
        for (Field declaredField : declaredFields) {
            try {
                ...
                String name = declaredField.getName();
                if (name.contains("To") && !value.toString().equals("-1")) {
                    System.out.println(loc + "       " + name + ILogger.MSG_SEPARATOR + value);
                }
            } catch (IllegalAccessException e) {
            }
        }
