...
public class MotionPaths implements Comparable<MotionPaths> {
    ...
    void initCartesian(KeyPosition c, MotionPaths startTimePoint, MotionPaths endTimePoint) {
        ...
        float f5 = startTimePoint.f33x;
        float startCenterX = f5 + (f2 / 2.0f);
        float position2 = startTimePoint.f34y;
        float startCenterY = position2 + (f4 / 2.0f);
        float endCenterX = endTimePoint.f33x + (f / 2.0f);
        float endCenterY = endTimePoint.f34y + (f3 / 2.0f);
        float pathVectorX = endCenterX - startCenterX;
        float pathVectorY = endCenterY - startCenterY;
        this.f33x = (int) ((f5 + (pathVectorX * position)) - ((scaleX * scaleWidth) / 2.0f));
        this.f34y = (int) ((position2 + (pathVectorY * position)) - ((scaleY * scaleHeight) / 2.0f));
        ...
        this.mMode = 2;
        this.f33x = (int) (((startTimePoint.f33x + (pathVectorX * dxdx)) + (pathVectorY * dxdy)) - ((scaleX * scaleWidth) / 2.0f));
        this.f34y = (int) (((startTimePoint.f34y + (pathVectorX * dydx)) + (pathVectorY * dydy)) - ((scaleY * scaleHeight) / 2.0f));
    ...
    void initScreen(int parentWidth, int parentHeight, KeyPosition c, MotionPaths startTimePoint, MotionPaths endTimePoint) {
        ...
        this.mMode = 3;
        if (!Float.isNaN(c.mPercentX)) {
            this.f33x = (int) (c.mPercentX * ((int) (parentWidth - this.width)));
        }
        if (!Float.isNaN(c.mPercentY)) {
            this.f34y = (int) (c.mPercentY * ((int) (parentHeight - this.height)));
        }
        this.mKeyFrameEasing = Easing.getInterpolator(c.mTransitionEasing);
        this.mPathMotionArc = c.mPathMotionArc;
    }
    ...
    void initPath(KeyPosition c, MotionPaths startTimePoint, MotionPaths endTimePoint) {
        ...
        float startCenterX = (startTimePoint.width / 2.0f) + startTimePoint.f33x;
        float startCenterY = startTimePoint.f34y + (startTimePoint.height / 2.0f);
        float endCenterX = endTimePoint.f33x + (endTimePoint.width / 2.0f);
        float endCenterY = endTimePoint.f34y + (endTimePoint.height / 2.0f);
        float pathVectorX = endCenterX - startCenterX;
        float pathVectorY = endCenterY - startCenterY;
        this.f33x = (int) ((f + (pathVectorX * path)) - ((scaleX * scaleWidth) / 2.0f));
        this.f34y = (int) ((f3 + (pathVectorY * path)) - ((scaleY * scaleHeight) / 2.0f));
        this.width = (int) (f2 + (scaleX * scaleWidth));
        ...
        this.mMode = 1;
        float f4 = (int) ((startTimePoint.f33x + (pathVectorX * path)) - ((scaleX * scaleWidth) / 2.0f));
        this.f33x = f4;
        float f5 = (int) ((startTimePoint.f34y + (pathVectorY * path)) - ((scaleY * scaleHeight) / 2.0f));
        this.f34y = f5;
        this.f33x = f4 + normalX;
        this.f34y = f5 + normalY;
        this.mKeyFrameEasing = Easing.getInterpolator(c.mTransitionEasing);
        this.mPathMotionArc = c.mPathMotionArc;
    }
    ...
    public void different(MotionPaths points, boolean[] mask, String[] custom, boolean arcMode) {
        ...
        int c2 = c + 1;
        mask[c] = mask[c] | diff(this.f33x, points.f33x) | arcMode;
        int c3 = c2 + 1;
        mask[c2] = mask[c2] | diff(this.f34y, points.f34y) | arcMode;
        int c4 = c3 + 1;
    ...
    public void fillStandard(double[] data, int[] toUse) {
        float[] set = {this.position, this.f33x, this.f34y, this.width, this.height, this.mPathRotate};
        int c = 0;
        for (int i = 0; i < toUse.length; i++) {
