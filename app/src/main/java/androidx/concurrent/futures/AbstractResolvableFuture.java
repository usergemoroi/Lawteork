...
public abstract class AbstractResolvableFuture<V> implements ListenableFuture<V> {
    ...
    public final V get(long timeout, TimeUnit unit) throws InterruptedException, TimeoutException, ExecutionException {
        ...
        String message = "Waited " + timeout + ILogger.MSG_SEPARATOR + unit.toString().toLowerCase(Locale.ROOT);
        if (remainingNanos + SPIN_THRESHOLD_NANOS < 0) {
            ...
            if (overWaitUnits > 0) {
                String message3 = message2 + overWaitUnits + ILogger.MSG_SEPARATOR + unitString;
                if (shouldShowExtraNanos) {
                    message3 = message3 + ",";
                }
                message2 = message3 + ILogger.MSG_SEPARATOR;
            }
            if (shouldShowExtraNanos) {
    ...
    private static final class SafeAtomicHelper extends AtomicHelper {
        ...
        boolean casWaiters(AbstractResolvableFuture<?> future, Waiter expect, Waiter update) {
            return C0137xc40028dd.m207m(this.waitersUpdater, future, expect, update);
        }

        @Override // androidx.concurrent.futures.AbstractResolvableFuture.AtomicHelper
        boolean casListeners(AbstractResolvableFuture<?> future, Listener expect, Listener update) {
            return C0137xc40028dd.m207m(this.listenersUpdater, future, expect, update);
        }

        @Override // androidx.concurrent.futures.AbstractResolvableFuture.AtomicHelper
        boolean casValue(AbstractResolvableFuture<?> future, Object expect, Object update) {
            return C0137xc40028dd.m207m(this.valueUpdater, future, expect, update);
        }
    }
