...
public final class WindowInsetsAnimationCompat {
    ...
    public static final class BoundsCompat {
        ...
        public BoundsCompat inset(Insets insets) {
            return new BoundsCompat(WindowInsetsCompat.insetInsets(this.mLowerBound, insets.left, insets.f255top, insets.right, insets.bottom), WindowInsetsCompat.insetInsets(this.mUpperBound, insets.left, insets.f255top, insets.right, insets.bottom));
        }
    ...
    public static class Impl21 extends Impl {
        ...
        static void setCallback(View view, Callback callback) {
            Object userListener = view.getTag(C0175R.C0178id.tag_on_apply_window_listener);
            if (callback == null) {
                view.setTag(C0175R.C0178id.tag_window_insets_animation_callback, null);
            ...
            view.setTag(C0175R.C0178id.tag_window_insets_animation_callback, proxyListener);
            if (userListener == null) {
        ...
        static BoundsCompat computeAnimationBounds(WindowInsetsCompat targetInsets, WindowInsetsCompat startingInsets, int mask) {
            ...
            Insets lowerBound = Insets.m204of(Math.min(targetInsetsInsets.left, startingInsetsInsets.left), Math.min(targetInsetsInsets.f255top, startingInsetsInsets.f255top), Math.min(targetInsetsInsets.right, startingInsetsInsets.right), Math.min(targetInsetsInsets.bottom, startingInsetsInsets.bottom));
            Insets upperBound = Insets.m204of(Math.max(targetInsetsInsets.left, startingInsetsInsets.left), Math.max(targetInsetsInsets.f255top, startingInsetsInsets.f255top), Math.max(targetInsetsInsets.right, startingInsetsInsets.right), Math.max(targetInsetsInsets.bottom, startingInsetsInsets.bottom));
        ...
        static WindowInsetsCompat interpolateInsets(WindowInsetsCompat target, WindowInsetsCompat starting, float fraction, int typeMask) {
            ...
            for (int i = 1; i <= 256; i <<= 1) {
                if ((typeMask & i) == 0) {
                    ...
                    Insets interpolatedInsets = WindowInsetsCompat.insetInsets(targetInsets, (int) (((targetInsets.left - startingInsets.left) * (1.0f - fraction)) + 0.5d), (int) (((targetInsets.f255top - startingInsets.f255top) * (1.0f - fraction)) + 0.5d), (int) (((targetInsets.right - startingInsets.right) * (1.0f - fraction)) + 0.5d), (int) (((targetInsets.bottom - startingInsets.bottom) * (1.0f - fraction)) + 0.5d));
        ...
        static WindowInsets forwardToViewIfNeeded(View v, WindowInsets insets) {
            if (v.getTag(C0175R.C0178id.tag_on_apply_window_listener) != null) {
                return insets;
            }
        ...
        static Callback getCallback(View child) {
            Object listener = child.getTag(C0175R.C0178id.tag_window_insets_animation_callback);
