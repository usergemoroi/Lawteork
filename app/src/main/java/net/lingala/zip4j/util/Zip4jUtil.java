...
public class Zip4jUtil {
    ...
    private static long epochToDosTime(long time) {
        ...
        return (cal.get(13) >> 1) | ((year - 1980) << 25) | ((cal.get(2) + 1) << 21) | (cal.get(5) << 16) | (cal.get(11) << 11) | (cal.get(12) << 5);
    }

    public static long dosToExtendedEpochTme(long dosTime) {
        long time = dosToEpochTime(dosTime);
        return (dosTime >> 32) + time;
    }

    private static long dosToEpochTime(long dosTime) {
        int sec = (int) ((dosTime << 1) & 62);
        int min = (int) ((dosTime >> 5) & 63);
        int hrs = (int) ((dosTime >> 11) & 31);
        int day = (int) ((dosTime >> 16) & 31);
        int mon = (int) (((dosTime >> 21) & 15) - 1);
        int year = (int) (((dosTime >> 25) & 127) + 1980);
        Calendar cal = Calendar.getInstance();
    ...
    public static int readFully(InputStream inputStream, byte[] bufferToReadInto) throws IOException {
        ...
        if (readLen != bufferToReadInto.length && (readLen = readUntilBufferIsFull(inputStream, bufferToReadInto, readLen)) != bufferToReadInto.length) {
